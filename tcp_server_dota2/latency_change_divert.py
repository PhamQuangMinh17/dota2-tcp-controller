...
- In the default 'rtt' mode, a requested latency of N ms is interpreted as an RTT
  target increase of N ms, so the delay is split across outbound and inbound packets
  instead of being applied fully to both directions.
...
class DivertConfig:
    ...
    # How to interpret requested latency values:
    #   - "rtt" (default): a requested value of N ms means "increase RTT by N ms".
    #     If both inbound and outbound are delayed, packets are split across directions
    #     so the total round-trip increase is N rather than 2N.
    #   - "per_direction": preserve the original behavior and delay each enabled
    #     direction by the full requested value. If both directions are enabled, the
    #     RTT increase is approximately 2N.
    latency_mode: str = "rtt"
...
class DivertLatencyController:
    ...
    def _normalized_latency_mode(self) -> str:
        mode = str(self.cfg.latency_mode).strip().lower()
        if mode not in ("rtt", "per_direction"):
            raise ValueError("DivertConfig.latency_mode must be 'rtt' or 'per_direction'.")
        return mode

    def _packet_delay_ms(self, requested_ms: int, outbound: bool) -> int:
        """
        Translate a requested latency value into the delay applied to one packet.

        In 'rtt' mode, when both inbound and outbound directions are enabled, the
        requested value is split across both directions so the total round-trip
        increase is equal to the requested latency. For odd values, outbound gets
        the extra 1 ms (ceil), inbound gets floor.
        """
        requested_ms = max(int(requested_ms), 0)
        if requested_ms <= 0:
            return 0

        delay_inbound = bool(self.cfg.delay_inbound)
        delay_outbound = bool(self.cfg.delay_outbound)

        if outbound and not delay_outbound:
            return 0
        if (not outbound) and not delay_inbound:
            return 0

        mode = self._normalized_latency_mode()
        if mode == "per_direction":
            return requested_ms

        # mode == "rtt"
        if delay_inbound and delay_outbound:
            return (requested_ms + 1) // 2 if outbound else requested_ms // 2

        # Only one direction is enabled, so apply the full requested RTT increase
        # to that enabled direction.
        return requested_ms
    ...
    def _rx_loop(self) -> None:
        ...
        while True:
            ...
            with self._cv:
                ...
                if not pass_through:
                    pkt_delay_ms = self._packet_delay_ms(lat_ms, outbound)
                    if pkt_delay_ms <= 0:
                        pass_through = True
                else:
                    pkt_delay_ms = 0
                ...
                if pass_through:
                    ...
                    self._buf.append(_BufferedPacket(_monotonic(), pkt, addr_bytes, pkt_delay_ms))
    ...
    def _tx_loop(self) -> None:
        ...
        while True:
            ...
            with self._cv:
                ...
                bp0 = self._buf[0]
                pkt_delay_ms = int(bp0.delay_ms)
                if pkt_delay_ms <= 0:
                    # If packet delay is zero, flush immediately (still keeps ordering).
                    pending_one = self._buf.popleft()
                    wait_sec = 0.0
                else:
                    now = _monotonic()
                    due = bp0.t_capture + (pkt_delay_ms / 1000.0)
                    if now >= due:
                        pending_one = self._buf.popleft()
                        wait_sec = 0.0
                    else:
...
def _cli() -> int:
    ...
    p.add_argument("--latency-mode", choices=["rtt", "per_direction"], default="rtt", help="Interpret latency as RTT increase (rtt) or per-direction delay (per_direction)")
    ...
    try:
        print(f"[windivert] Running with latency={args.latency} ms, filter={args.filter}, latency_mode={args.latency_mode}")
