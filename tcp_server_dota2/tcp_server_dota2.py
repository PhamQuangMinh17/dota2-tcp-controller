...
def run_server(
    ...
    controller = DivertLatencyController(
        DivertConfig(
            ...
            delay_inbound=delay_inbound,
            delay_outbound=delay_outbound,
    ...
    print(f"[tcp_server] Latency mode: {latency_mode}")
    print(f"[tcp_server] Delay directions: inbound={delay_inbound}, outbound={delay_outbound}")
...
def _cli() -> int:
    ...
    p.add_argument("--latency-mode", choices=["rtt", "per_direction"], default="rtt", help="Interpret requested latency as RTT increase (rtt) or as full per-direction delay (per_direction)")
    p.add_argument("--delay-inbound", dest="delay_inbound", action="store_true", default=True, help="Delay inbound packets (default: True)")
    p.add_argument("--no-delay-inbound", dest="delay_inbound", action="store_false", help="Do not delay inbound packets")
    p.add_argument("--delay-outbound", dest="delay_outbound", action="store_true", default=True, help="Delay outbound packets (default: True)")
    p.add_argument("--no-delay-outbound", dest="delay_outbound", action="store_false", help="Do not delay outbound packets")
    ...
    return run_server(
        ...
        delay_inbound=args.delay_inbound,
        delay_outbound=args.delay_outbound,
    )
